  ...
  public:
    ...
    double* estimateSpeed(unsigned long* time, double* acc, double* pos)
    {
      if (this->firstExecution > 2)
      {
        ...
        this->speedEstIntegrated  = this->speedEst + (*acc - this->staticAccOffset) * deltaT;
        this->speedEstDifferentiator = (*pos - this->lastPosValue) / deltaT;
        
        
        this->speedEst = (1 - this->alpha) * this->speedEstIntegrated + (this->alpha) * this->speedEstDifferentiator;
      }
      else
      {
        this->firstExecution++;
